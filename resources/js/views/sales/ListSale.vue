  ...
  <v-container>
    ...
    <v-row v-if="!loadingData">
      ...
      >
        ...
        <print-facture-letter
          v-if="showShowModal && printer === 'letter'"
        />
        <app-data-table
          :id="id"
          :title="
...
export default {
      ...
      return [
        {
          ...
          content: this.$vuetify.lang.t(
            '$vuetify.helpSales.listStep1'
          ),
          params: {
            placement: 'left'
          }
        },
        {
          ...
          content: this.$vuetify.lang.t(
            '$vuetify.helpSales.listStep2'
          )
        },
        {
          target: '#' + this.id + ' .v-data-table__wrapper',
          content: this.$vuetify.lang.t(
            '$vuetify.helpSales.listStep3'
          )
        }
      ]
    }
  ...
  methods: {
    ...
    editSaleHandler ($event) {
      let show = true
      if (
        this.sales.filter(sl => sl.id === $event)[0].state ===
                'accepted'
      ) {
        if (
          this.sales.filter(sl => sl.id === $event)[0].refounds
            .length > 0
        ) {
          show = false
          this.showMessage(
