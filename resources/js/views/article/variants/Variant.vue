<template>
    <v-data-table :headers="headers" :items="variantsValues" sort-by="calories" class="elevation-1">
        <template v-slot:top>
            <v-toolbar flat>
                <v-chip v-for="tag in variants" close
                        close-icon="mdi-delete"
                        @click:close="removeChips(tag)"
                        color="success"
                        text-color="white">
                    <v-icon left @click="editChips(tag)">mdi-pencil</v-icon>
                    {{ tag.name }}
                </v-chip>
                <v-spacer></v-spacer>
                            ...
                            <v-container>
                                <v-row>
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field v-model="editedItem.name" style="margin-top: 14px"
                                                      :label="$vuetify.lang.t('$vuetify.variants.name')"
                                        ></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="8">
                                        <v-combobox multiple v-model="select"
                                                    :label="$vuetify.lang.t('$vuetify.variants.options')"
                                                    append-icon chips deletable-chips
                                                    class="tag-input" @keyup.tab="updateTags" @paste="updateTags">
                                        </v-combobox>
                                    </v-col>
                                </v-row>
                            </v-container>
                ...
                <v-dialog v-model="dialogDelete" max-width="500px">
                    <v-card>
                        ...
                        <v-card-actions>
                            ...
                            <v-btn
                                class="mb-2"
                                color="error" @click="closeDelete">
                                <v-icon>mdi-close</v-icon>
                                {{ $vuetify.lang.t('$vuetify.actions.cancel') }}
                            </v-btn>
                            <v-btn
                                class="mb-2"
                                color="primary" @click="deleteItemConfirm">
                                <v-icon>mdi-check</v-icon>
...
export default {
    ...
    methods: {
        ...
        myComb() {
            let data = this.variants.reverse();
            let result = [];
            let localResult = [];
