            ...
            <h4>
              ...
              {{
                `${user.company.currency +
                  " " +
                  parseFloat(article.cost).toFixed(2)}`
              }}
            </h4>
            <v-spacer />
            <h4>
  ...
  async created () {
    ...
    await this.getArticles().then(() => {
      this.articles.forEach(value => {
        this.ref =
                    parseFloat(value.ref) > parseFloat(this.ref)
                      ? value.ref
                      : this.ref
        if (!value.article_id) {
  ...
  methods: {
    ...
    save () {
      if (this.editedIndex > -1) {
        Object.assign(
          this.article.composite[this.editedIndex],
          this.editedItem
        )
      } else {
    ...
    selectArticle (item) {
      if (
        this.article.composites.filter(
          art => art.composite_id === item.composite_id
        ).length === 0
      ) {
