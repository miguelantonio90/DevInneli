...
const mutations = {
	...
	[MODIFIERS_CREATED] (state) {
		...
		this._vm.$Toast.fire({
			...
			title: this._vm.$language.t(
				'$vuetify.messages.success_add', [this._vm.$language.t('$vuetify.menu.modifiers')]
			)
		})
	},
	[MODIFIERS_EDIT] (state, modifierId) {
		state.editModifier = Object.assign({}, state.modifiers
			.filter(node => node.id === modifierId)
			.shift()
		)
	},
	[MODIFIERS_UPDATED] (state) {
		...
		this._vm.$Toast.fire({
			icon: 'success',
			title: this._vm.$language.t(
				'$vuetify.messages.success_up', [this._vm.$language.t('$vuetify.menu.modifiers')]
			)
		})
	},
	[MODIFIERS_DELETE] (state, error) {
		...
		this._vm.$Toast.fire({
			icon: 'success',
			title: this._vm.$language.t(
				'$vuetify.messages.success_del', [this._vm.$language.t('$vuetify.menu.modifiers')]
			)
		})
	},
	[FAILED_MODIFIERS] (state, error) {
		...
		this._vm.$Toast.fire({
			icon: 'error',
			title: this._vm.$language.t(
				'$vuetify.messages.failed_catch', [this._vm.$language.t('$vuetify.menu.modifiers')]
			)
		})
	}
}
...
const actions = {
	...
	async getModifiers ({ commit, dispatch }) {
	...
	async createModifiers ({ commit, dispatch }, newModifier) {
	...
	async updateModifiers ({ commit, dispatch }, editModifier) {
		commit(ENV_DATA_PROCESS, true)
		await api
	...
	async deleteModifiers ({ commit, dispatch }, modifierId) {
		await api
