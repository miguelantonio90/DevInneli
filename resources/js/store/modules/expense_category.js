...
const mutations = {
  ...
  [EXPENSE_CREATED] (state) {
	...
	this._vm.$Toast.fire({
	  ...
	  title: this._vm.$language.t('$vuetify.messages.success_add', [
		this._vm.$language.t('$vuetify.menu.expense_category')
	  ])
	})
  },
  [EXPENSE_EDIT] (state, categoryId) {
	state.editCategory = Object.assign(
	  {},
	  state.categories.filter(node => node.id === categoryId).shift()
	)
  },
  ...
  async createCategory ({
	...
	await expenseCategory
	  ...
	  .then(data => {
		...
		dispatch('expenseCategory/getExpenseCategories', null, {
		  root: true
		})
		this.dispatch('auth/updateAccess', data)
	  })
