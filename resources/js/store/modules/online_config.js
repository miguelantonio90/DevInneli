...
const mutations = {
  ...
  [FETCHING_CONFIGS_BY_SHOP] (state, config) {
    ...
    if (config.id !== undefined) { state.ediConfig = config }
  },
  ...
  [CONFIG_EDIT] (state, articleId) {
    state.editConfig = Object.assign({}, state.configs
...
const actions = {
  ...
  async getConfigs ({ commit, dispatch }) {
  ...
  async getConfigByShop ({ commit, dispatch }, shopId) {
    commit(CONFIG_TABLE_LOADING, true)
    // noinspection JSUnresolvedVariable
    await apiConfig
  ...
  async createConfig ({ commit, dispatch }, newConfig) {
  ...
  async updateConfig ({ commit, dispatch }, articleE) {
    commit(ENV_DATA_PROCESS, true)
  ...
  async deleteConfig ({ commit, dispatch }, articleId) {
    await apiConfig
