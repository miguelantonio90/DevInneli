...
const actions = {
  ...
  openNewModal ({ commit }, { sale, article }) {
    commit(REFUND_NEW, { sale, article })
    commit(SWITCH_REFUND_NEW_MODAL, true)
  },
  async getRefunds ({ commit, dispatch }) {
  ...
  async updateRefund ({ commit, dispatch }, refundE) {
    commit(ENV_DATA_PROCESS, true)
  ...
  async deleteRefund ({ commit, dispatch }, refundId) {
    await refund
