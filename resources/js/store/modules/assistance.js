  ...
  [ASSIST_CREATED] (state) {
    ...
    this._vm.$Toast.fire({
	  ...
	  title: this._vm.$language.t('$vuetify.messages.success_add', [
        this._vm.$language.t('$vuetify.menu.assistance')
	  ])
    })
  },
  [ASSIST_EDIT] (state, assistanceId) {
    state.editAssistance = Object.assign(
	  {},
	  state.assistances.filter(node => node.id === assistanceId).shift()
    )
  },
