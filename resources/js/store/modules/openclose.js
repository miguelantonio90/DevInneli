...
const mutations = {
	...
	[OPEN_CLOSE_CREATED] (state) {
		...
		this._vm.$Toast.fire({
			...
			title: this._vm.$language.t(
				'$vuetify.messages.success_add', [this._vm.$language.t('$vuetify.menu.openclose')]
			)
		})
	},
	[OPEN_CLOSE_EDIT] (state, opencloseId) {
		state.editOpenClose = Object.assign({}, state.opencloses
			.filter(node => node.id === opencloseId)
			.shift()
		)
	},
	[OPEN_CLOSE_UPDATED] (state) {
		...
		this._vm.$Toast.fire({
			icon: 'success',
			title: this._vm.$language.t(
				'$vuetify.messages.success_up', [this._vm.$language.t('$vuetify.menu.openclose')]
			)
		})
	},
	...
	[OPEN_CLOSE_DELETE] (state) {
		...
		this._vm.$Toast.fire({
			icon: 'success',
			title: this._vm.$language.t(
				'$vuetify.messages.success_del', [this._vm.$language.t('$vuetify.menu.openclose')]
			)
		})
	},
	[FAILED_OPEN_CLOSE] (state, error) {
		...
		this._vm.$Toast.fire({
			icon: 'error',
			title: this._vm.$language.t(
				'$vuetify.messages.failed_catch', [this._vm.$language.t('$vuetify.menu.openclose')]
			)
		})
	}
}
...
const actions = {
	...
	async updateOpenClose ({ commit, dispatch }, editOpenClose) {
		await openclose
